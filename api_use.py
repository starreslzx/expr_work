...
2. 为每个话题生成：
   ...
   优先级（高/中/低）
   简要总结（3-5句话）
   相关的全部聊天记录都要保留，包括时间，人物，以及该人物说的话
   相关话题（与其他话题的关联）
...
6. 请确保返回的是有效的JSON格式
7. 不要把没用的聊天记录删除掉，全部聊天记录都要保留并归属在一个话题下面，比如一个话题是一段记录都在讨论的，该段所有的记录都要保留
8. 如果上传的文件没有时间，就默认为0000年00月00日"""
